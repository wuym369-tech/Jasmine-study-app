<template>
  <div class="min-h-screen bg-yellow-50">
    ...
    <div v-if="phase === 'setup'" class="max-w-2xl mx-auto px-4 py-8">
      <router-link to="/" class="text-green-600 hover:text-green-800 mb-6 inline-block text-sm">← 返回首页</router-link>
      <h1 class="text-2xl font-bold text-yellow-800 mb-1">💬 情境讨论模式</h1>
      <p class="text-yellow-600 text-sm mb-8">小组共同讨论农场情境，找出最佳应对方案</p>

      <!-- 人数推荐 -->
      <div class="bg-white rounded-2xl border border-yellow-200 p-5 mb-6">
        <h2 class="font-bold text-gray-700 mb-3">👥 依学生人数选择题数</h2>
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div class="text-yellow-600 font-bold">推荐 {{ rec.recommended }} 题</div>
      ...
      <!-- 自定题数 -->
      <div class="bg-white rounded-2xl border border-yellow-200 p-5 mb-6">
        <h2 class="font-bold text-gray-700 mb-3">🎯 或自定题目数量与类别</h2>
        <div class="flex items-center gap-3 mb-4">
          <span class="text-sm text-gray-600">总题数</span>
          ...
          <span class="text-sm text-gray-400">（最少3题，最多25题）</span>
        </div>

        <!-- 类别筛选 -->
        <div class="space-y-2">
          <p class="text-sm text-gray-600 mb-2">选择题目类别（可多选）：</p>
          <div class="flex flex-wrap gap-2">
            ...
            >
              {{ cat.icon }} {{ cat.label }}（{{ cat.count }}题）
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- 每题讨论时间 -->
      <div class="bg-white rounded-2xl border border-yellow-200 p-5 mb-6">
        <h2 class="font-bold text-gray-700 mb-3">⏱️ 每题讨论时间</h2>
      ...
      >
        开始讨论（共 {{ actualCount }} 题）
      </button>
      <p v-if="selectedCategories.length === 0" class="text-center text-sm text-red-400 mt-2">请至少选择一个类别</p>
    </div>
    ...
    <div v-if="phase === 'question'" class="max-w-2xl mx-auto px-4 py-6">
      ...
      <div class="flex items-center justify-between mb-4">
        <span class="text-sm text-gray-500">第 {{ currentIndex + 1 }} / {{ queue.length }} 题</span>
        <div class="flex items-center gap-2">
          <span class="text-2xl font-bold"
            :class="timeLeft <= 15 ? 'text-red-500' : 'text-yellow-600'">
            {{ timeLeft }}s
          </span>
          <span class="text-sm text-gray-400">讨论时间</span>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border-2 border-yellow-200 p-6 mb-5 shadow-sm">
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="text-xs text-gray-400">难度：{{ currentQ.difficulty }}</span>
          <span class="text-xs text-gray-400 ml-auto">相关角色：{{ currentQ.roles.join('、') }}</span>
        </div>
      ...
      <div v-if="showAnswer" class="bg-white rounded-2xl border-2 border-green-200 p-5 mb-4">
        <div class="flex items-center gap-2 mb-3">
          ...
          <h3 class="font-bold text-green-800">
            {{ selectedAnswer === currentQ.answer ? '答对了！' : `正确答案是 ${currentQ.answer}` }}
          </h3>
        </div>
    ...
    <div v-if="phase === 'result'" class="max-w-2xl mx-auto px-4 py-8">
      <div class="bg-white rounded-2xl border border-green-200 p-6 text-center mb-6 shadow-sm">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-2">讨论结束！</h2>
        <p class="text-gray-500 mb-4">共 {{ queue.length }} 题</p>
...
const categories = [
  { id: 'weather', label: '天气', icon: '🌦️', activeClass: 'border-blue-400 bg-blue-50 text-blue-700', count: 8 },
  { id: 'pest', label: '虫害', icon: '🐛', activeClass: 'border-red-400 bg-red-50 text-red-700', count: 6 },
